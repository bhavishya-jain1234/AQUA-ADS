...
const WaterBackground = () => {
  ...
  useEffect(() => {
    ...
    const waveLayers = isMobile
      ? [
          { color: DEEP_OCEAN, speed: 0.3 * speedScale, amplitude: 52, frequency: 0.009, offset: 0 },
          { color: OCEAN_BLUE, speed: 0.6 * speedScale, amplitude: 34, frequency: 0.013, offset: 250 },
        ]
      : [
          { color: DEEP_OCEAN, speed: 0.3 * speedScale, amplitude: 64, frequency: 0.0085, offset: 0 },
          { color: OCEAN_BLUE, speed: 0.6 * speedScale, amplitude: 42, frequency: 0.012, offset: 260 },
          { color: CYAN, speed: 1.0 * speedScale, amplitude: 22, frequency: 0.019, offset: 520 },
        ];
    ...
    const drawWave = (layer, phase) => {
      const baseY = height * 0.64 + Math.sin(phase * 0.15) * 8 + layer.offset * 0.02;
      ctx.beginPath();
      ctx.moveTo(0, height);
